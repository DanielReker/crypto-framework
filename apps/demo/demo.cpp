...
void DemonstrateCsp(std::shared_ptr<ICsp> csp, const std::string& name) {
	try {
		...
		ddd = cert->VerifyCades(cadesBesDetached, CadesType::kBes);
		...
		ddd = cert->VerifyCades(cadesBesAttached, CadesType::kBes);
		...
		ddd = cert->VerifyCades(cadesXlDetached, CadesType::kBes);
		...
		ddd = cert->VerifyCades(cadesXlAttached, CadesType::kBes);
...
int main(int argc, char* argv[]) {
	...
	// const auto& encrypted = certs1[0]->Encrypt(file);
